...
/*
 * Build a DataConversionStrideInfo for an FP8_MULTI_DIM_STICK weight transfer.
 *
 * The 3D STL (device_size=[N/eps, K, eps], stride_map=[eps, K_sm, 1]) encodes
 * the compute layout.  For the DMA transfer the stick must be expanded into a
 * 2D pair [stick_inner=2, stick_outer=eps/2=64] whose byte arrangement in
 * device memory differs from a flat eps-element stick.  get_device_stride_infos
 * cannot derive this because the required stride_map values exceed the host
 * tensor's element count, so we construct the DataConversionStrideInfo
 * directly.
 *
...
auto generate_dci(const at::Tensor* cpu_tensor, const at::Tensor* dev_tensor,
  ...
  if (stl.element_arrangement == ElementArrangement::FP8_MULTI_DIM_STICK) {
    // FP8 weight with 2D stick: bypass get_device_stride_infos and build the
    // DataConversionStrideInfo directly from the 3D STL parameters.
    // input_shape_ and output_shape_ are innermost-first (4D expanded layout).
    const int64_t eps = stl.elems_per_stick();
    const int64_t si = 2;
    const int64_t so = eps / si;
    const int64_t K = stl.device_size[1];
