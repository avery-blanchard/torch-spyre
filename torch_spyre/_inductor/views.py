...
def compute_coordinates(
    ...
    def add_term(var, step, limit):
        ...
        if primary_stride > 0:
            # Detect multi-stick: primary is stick dim (n-1) and outermost stride
            # equals primary stride (indicates padding beyond single stick)
            is_stick_var = primary_dim == n - 1
            outermost_stride = _concretize_for_cmp(stride[0])
            is_multistick = (
                is_stick_var
                and outermost_stride == primary_stride
                and _concretize_for_cmp(size[0]) > 1
            )

            if is_multistick and concrete_limit > outermost_stride:
                # Stick variable range exceeds stick boundary: split coordinate
                # into floor(var/stride) and Mod(var, stride)
                stick_size = primary_stride
                coordinates[0] = var * step // stick_size
                coordinates[primary_dim] += var * step % stick_size // primary_stride
            elif next_stride[primary_dim] < concrete_limit:
