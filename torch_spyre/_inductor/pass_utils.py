...
def indirect_forbidden_split_syms(op: "ComputedBuffer") -> set[Symbol]:
    ...
    # (2) With propagate_layouts now rounding indirect-output entry dims up-front
    # to a whole-stick extent at layout construction time, split safety on the
    # index-entry dimension is guaranteed: the output can always hold a
    # whole-stick slice. No need to forbid partial-stick splits anymore.
    # (Kept for reference: the old logic forbade splits when output was not
    # stick-aligned, a defensive check that is now unconditionally satisfied.)
