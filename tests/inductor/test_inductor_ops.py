...
class TestOps(unittest.TestCase, metaclass=ParameterizedTestMeta):
        ...
        ("test_dequantize_fp8_with_scale", "test_dequantize_fp8_with_scale_cpu"): {
            "param_sets": {
                "shape_128_128_scale_0.01": (
                    cached_randn((128, 128), dtype=torch.float16, scale=1.0),
                    torch.tensor([0.01], dtype=torch.float16),
                ),
                "shape_4096_1024_scale_0.1": (
                    cached_randn((4096, 1024), dtype=torch.float16, scale=5.0),
        ...
        (
            ...
            "param_sets": {
                "shape_1_128_512_scale_0.01": (
                    cached_randn((1, 128, 512), dtype=torch.float16, scale=1.0),
                    torch.tensor([0.01], dtype=torch.float16),
                ),
                "shape_4_128_512_scale_0.1": (
                    cached_randn((4, 128, 512), dtype=torch.float16, scale=5.0),
                    torch.tensor([0.1], dtype=torch.float16),
                ),
                "shape_1_128_1024_scale_0.5": (
                    cached_randn((1, 128, 1024), dtype=torch.float16, scale=50.0),
                    torch.tensor([0.5], dtype=torch.float16),
                ),
                "shape_1_128_2048_scale_1.0": (
                    cached_randn((1, 128, 2048), dtype=torch.float16, scale=100.0),
                    torch.tensor([1.0], dtype=torch.float16),
                ),
                "shape_1_128_4096_scale_2.0": (
                    cached_randn((1, 128, 4096), dtype=torch.float16, scale=200.0),
                    torch.tensor([2.0], dtype=torch.float16),
                ),
            },
        },
    ...
    def test_quantize_fp8_with_scale_cpu(self):
        x = cached_randn((1, 2, 8), dtype=torch.float16, scale=0.5)
        scale_value = 0.5
        ...
        compare_with_pytorch(spyre_fn, pytorch_fn, x, scale, atol=0.1, rtol=0.0)
    ...
    def test_dequantize_fp8_with_scale_cpu(self, x, scale):
        ...
        compare_with_pytorch(spyre_fn, pytorch_fn, x, scale, atol=64, rtol=0.1)
